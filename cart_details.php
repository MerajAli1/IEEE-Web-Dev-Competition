	...
	if(isset($_SESSION['user'])){
		if(isset($_SESSION['cart'])){
			foreach($_SESSION['cart'] as $row){
				$stmt = $conn->prepare("SELECT id, COUNT(*) AS numrows FROM cart WHERE user_id=:user_id AND product_id=:product_id GROUP BY id");
		...
		try{
			...
			foreach($stmt as $row){
				...
				$response['output'] .= "
					...
					<td><img src='".$image."' width='30px' height='30px'></td>
						...
						<td>".number_format($row['price'], 2)." Rs </td>
						
						<td> ".number_format($subtotal, 2)." Rs </td>
			...
			$response['output'] .= "
				<tr>
					...
					<td><b> ".number_format($total, 2)." Rs</b></td>
				<tr>
			";
				...
				$response['output'] .= "
					<tr>			
						...
						<td class='input-group'>
            				...
            				<input type='text' style='text-align: -webkit-center;' 'class='form-control' value='".$row['quantity']."' id='qty_".$row['productid']."'>
				            <span class='input-group-btn'>
				                <button type='button' id='add' class='btn btn-default btn-flat add' data-id='".$row['productid']."'><i class='fa fa-plus'></i>
				                </button>
				            </span>
						</td>
						<td> ".number_format($product['price'], 2)." Rs</td>
						<td> ".number_format($subtotal, 2)." Rs</td>
