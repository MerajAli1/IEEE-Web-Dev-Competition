	...
	if(isset($_SESSION['user'])){
		try{
			...
			foreach($stmt as $row){
				...
				$output['list'] .= "
					<li style='height: 13vh;'>
						<a href='product.php?product=".$row['slug']."'>
							<div class='pull-left' style='width: 30%; margin-right: 10%;'>
								<img style='width: 100%;' src='".$image."' class='thumbnail' alt='User Image'>
							</div>
							<h4>
		                        <b>".$row['catname']."</b>
		...
		if(empty($_SESSION['cart'])){
			...
			foreach($_SESSION['cart'] as $row){
				...
				$output['list'] .= "
					<li style='height: 16vh; margin-bottom: 4vh;'>
